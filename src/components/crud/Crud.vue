        ...
        <form class="form-horizontal" @submit.prevent.default>
          <div class="box-body">
            ...
            <template v-for="field in configuration.form.fields">
              <endereco v-if="field.type == 'endereco'" v-model="model"></endereco>
              <div v-else class="form-group">
                <label class="col-sm-3 control-label">{{ field.title }}</label>
                <div class="col-sm-9">
                  <input v-model.number="model[field.model]" class="form-control" v-if="field.type == 'money'" type="number" :min="field.min || 0" step="any" />
                  <input v-model="model[field.model]" class="form-control" v-else-if="field.type == 'integer'" type="number" :min="field.min || 0" step="1" />
                  <select class="form-control" v-model="model[field.model]" v-else-if="field.type == 'select' && field.options" :required="field.required">
                    <option v-for="opt in field.options" :value="opt.value">{{ opt.text }}</option>
                  </select>               
                  <input v-model="model[field.model]" class="form-control" v-else type="text" :placeholder="field.title" :required="field.required">            
                </div>
              </div>
            </template>
          </div>
